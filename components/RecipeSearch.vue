<template>
    <div class="mb-3">
        <form id="recipe-search" class="mx-auto bg-[#dedcd9] flex rounded-full focus-within:shadow focus-within:bg-[#1a4e48]/25 focus-within:text-black transition-colors placeholder:text-white" :class="{focused: focused}" @submit.prevent> 
            <button class="px-3 rounded-l-full hover:bg-black/20" @click="do_search">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16">
                    <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"/>
                </svg>
            </button>
            <input @keydown.enter="do_search" @focus="focused = true" @blur="focused = false" class="w-full h-[44px] py-2 px-1 bg-transparent color-black focus:outline-0 placeholder:text-black" type="text" id="search" name="search" aria-label="Search our recipes" placeholder="Search..." v-model="search_query">
...
<script setup>
    ...
    let search_query = ref(route.query.search)
    ...
    let do_search = (e) => {
        ...
        if(router.currentRoute.value.name !== 'recipes-search') {
            window.location.href = window.location.origin + `/recipes/search/?search=${search_query.value}`
            return
        }
